...
`geo-aeo-audit` 是一個唯讀的 CLI，用來對公開 HTTP(S) 頁面做可重現的 GEO／AEO 觀測。預設的 `audit` 命令檢查有界的技術可存取性、爬蟲政策、探索訊號、靜態內容、實體與佐證訊號，全程不呼叫任何模型 API。選用的 `probe` 命令會呼叫你指定的 provider API，觀測它是否真的搜尋並引用目標頁面；這可能產生 provider 費用。
...
兩個命令各有自己的選項與 `--help`，一個命令的旗標不能用在另一個命令上。
...
| 代碼 | 意義 |
|---|---|
| 0 | audit 完成且未達門檻；或 probe 完成並產出可回報的 attempt 結果。 |
| 1 | audit 完成但達到 `--fail-on` 門檻。probe 不會回傳 1；provider 錯誤與 timeout 都記錄在結果中。 |
| 2 | CLI 用法或設定錯誤。 |
| 3 | 命令或要求的報告輸出無法完成。 |
...
測試套件會先建置 CLI，然後執行單元測試、合成 provider fixture 整合測試、本機 HTTP fixture、JSON Schema 相容性、HTML 安全性與 CLI 端到端測試。測試不會存取真實公開網站、不呼叫付費模型 API、也不修改被稽核的網站。
...
| 1 | Audit completed but the selected `--fail-on` threshold was met. Probe never returns 1; provider errors and timeouts are recorded in the result. |
